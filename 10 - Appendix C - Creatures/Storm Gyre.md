---
name: Storm Gyre
slug: storm-gyre-page
---

```Monster {.two-column}
name: Storm Gyre
slug: storm-gyre
size: Medium
type: Elemental (Air)
alignment: neutral
ac: 18 (natural)
hp: 135 (9d10 + 35)
speed: 0 ft., fly 60 ft. (hover)
str: 10
dex: 17
con: 12
int: 5
wis: 13
cha: 5
senses: Passive Perception 12, Darkvision 60 ft.
damageResistances: Lightning, Thunder; Bludgeoning, Piercing, and Slashing from Nonmagical Attacks
damageImmunities: Poison
conditionImmunities: Exhaustion, Grappled, Paralyzed, Petrified, Poisoned, Prone, Restrained, Unconscious
languages: Auran
challenge: 8
token: StormGyreToken.png
image: StormGyre.jpg
traits:
  - name: Essence of Air
    description: "As a being of pure air, the gyre cannot be knocked prone, is immune to disease and poison, and cannot be paralyzed, petrified, or put to sleep. It does not need to sleep, eat, or breathe."
actions:
  - name: Percuss
    description: "Melee Weapon Attack: +7 to hit, reach 5 ft., one target. Hit: 19 (2d10 + 8) thunder damage."
  - name: Conduction
    description: "The Storm Gyre picks a target within 60 feet. That target, and all other targets within 10 feet of that target, must make a DC 14 Dexterity saving throw. All targets take 22 (5d8) lightning damage on a failed save, or half as much on a successful one."
column-after: actions
column-after-property: Percuss
```